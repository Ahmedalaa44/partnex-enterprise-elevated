...
if (!authToken) {
  ...
  console.log('Please set: export SANITY_AUTH_TOKEN=<your-token>');
  process.exit(1);
}

const baseUrl = `https://api.sanity.io/v${API_VERSION}/projects/${PROJECT_ID}/datasets/${DATASET}/documents`;
...
const documents = [
  ...
  { _type: 'service', title: 'Cloud Solutions', desc: 'Scalable cloud architectures that accelerate innovation, agility and time to value.', icon: 'Cloud', order: 1 },
  { _type: 'service', title: 'Cybersecurity', desc: 'Advanced security programs that reduce risk and protect what matters most.', icon: 'ShieldCheck', order: 2 },
  { _type: 'service', title: 'Data Protection', desc: 'Backup, disaster recovery and resilience engineered for continuity.', icon: 'DatabaseBackup', order: 3 },
  { _type: 'service', title: 'Infrastructure', desc: 'Modern infrastructure designed for performance, reliability and scale.', icon: 'Server', order: 4 },
  { _type: 'service', title: 'Modern Workplace', desc: 'Empowering teams to collaborate and work smarter from anywhere.', icon: 'Laptop', order: 5 },
  { _type: 'service', title: 'Managed Services', desc: 'Proactive 24/7 monitoring, management and continuous optimization.', icon: 'Activity', order: 6 },
  { _type: 'service', title: 'Networking & Communication', desc: 'High-performance connectivity that keeps your operations ahead.', icon: 'Network', order: 7 },
  { _type: 'service', title: 'Consulting & Advisory', desc: 'Expert guidance aligning technology decisions with your business goals.', icon: 'Compass', order: 8 },
...
async function migrate() {
  console.log('🚀 Starting automatic content migration...\n');
  let created = 0;
  let skipped = 0;
  ...
  for (const doc of documents) {
    ...
    try {
      ...
      if (response.ok) {
        ...
        console.log(`✅ ${docTitle} (${docType})`);
        created++;
      } else if (response.status === 409) {
        console.log(`⚠️  ${docTitle} already exists`);
        skipped++;
      } else {
        const error = await response.json();
        console.error(`❌ ${docTitle}: ${error.message}`);
      }
    } catch (error) {
      console.error(`❌ Error creating ${docTitle}:`, error.message);
    }
  }
